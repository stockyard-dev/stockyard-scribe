...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/notes", s.list)
	s.mux.HandleFunc("POST /api/notes", s.create)
	s.mux.HandleFunc("GET /api/notes/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/notes/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-scribe", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"notes": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Note{} }
	wj(w, map[string]any{"notes": items, "count": len(items)})
}
