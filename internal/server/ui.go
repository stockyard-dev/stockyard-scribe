...
async function load(){var r=await fetch(A+'/recordings').then(function(r){return r.json()});items=r.recordings||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.title||'').toLowerCase().includes(q)||(i.duration||'').toLowerCase().includes(q)||(i.source||'').toLowerCase().includes(q)||(i.transcript||'').toLowerCase().includes(q)});
...
if(i.duration)h+='<div class="item-sub">'+esc(i.duration)+'</div>';
...
if(i.source)h+='<span>'+esc(i.source)+'</span>';
if(i.transcript)h+='<span>'+esc(i.transcript)+'</span>';
if(i.summary)h+='<span>'+esc(i.summary)+'</span>';
if(i.tags)h+='<span>'+esc(i.tags)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.word_count)h+='<span>'+esc(i.word_count)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/recordings/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{title:"",duration:"",source:"",transcript:"",summary:"",tags:"",status:"",word_count:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' SCRIBE</h2>';
h+='<div class="fr"><label>Title *</label><input id="f-title" value="'+esc(i.title)+'"></div>';
h+='<div class="fr"><label>Duration</label><input id="f-duration" value="'+esc(i.duration)+'"></div>';
h+='<div class="fr"><label>Source</label><input id="f-source" value="'+esc(i.source)+'"></div>';
h+='<div class="fr"><label>Transcript</label><input id="f-transcript" value="'+esc(i.transcript)+'"></div>';
h+='<div class="fr"><label>Summary</label><input id="f-summary" value="'+esc(i.summary)+'"></div>';
...
h+='<div class="fr"><label>Word Count</label><input id="f-word-count" value="'+esc(i.word_count)+'"></div>';
...
async function submit(){var body={title:document.getElementById('f-title').value.trim(),duration:document.getElementById('f-duration').value.trim(),source:document.getElementById('f-source').value.trim(),transcript:document.getElementById('f-transcript').value.trim(),summary:document.getElementById('f-summary').value.trim(),tags:document.getElementById('f-tags').value.trim(),status:document.getElementById('f-status').value.trim(),word_count:document.getElementById('f-word-count').value.trim()};
if(editId){await fetch(A+'/recordings/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/recordings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
